...
// Lấy danh sách các danh mục để hiển thị trong select
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $conn = new mysqli('localhost', 'root', '', 'shop_shoes');
    ...
    // Tạo ID tự động cho nhà cung cấp
    // Lấy ID mới nhất để tạo ID tiếp theo
    $sql = "SELECT MAX(id_ncc) AS max_id FROM supllier";
    $result = $conn->query($sql);
    $row = $result->fetch_assoc();
    $last_id = $row['max_id'];

    // Tạo ID mới bằng cách tăng số cuối cùng
    $new_id = "SUP-" . str_pad((intval(substr($last_id, 4)) + 1), 3, "0", STR_PAD_LEFT);

    // Chuẩn bị câu lệnh INSERT
    $sqlInsert = "INSERT INTO supllier (id_ncc, TenNCC, DiaChi, SDT, MoTa, id_category) 
                  VALUES ('$new_id', '$TenNCC', '$DiaChi', '$SDT', '$MoTa', '$id_category')";

    // Thực thi câu lệnh
    if ($conn->query($sqlInsert) === TRUE) {
        // Chuyển hướng về trang danh sách nhà cung cấp sau khi thêm thành công
        header('Location: index.php');
        exit();
    } else {
        echo "Lỗi khi thêm nhà cung cấp: " . $conn->error;
    }
...
<div class="card shadow border-0 mb-7">
    ...
    <div class="table-responsive">
        <div class="panel-body">
            ...
            <input type="text" id="id_ncc" name="id_ncc" value="" hidden>
            ...
            <div class="form-group">
                ...
                <textarea required="true" class="form-control" id="MoTa" name="MoTa"></textarea>
            </div>
            ...
            <button class="btn btn-success" type="submit" onclick="addSupplier()">Lưu</button>
...
<script type="text/javascript">
    function addSupplier() {
        var option = confirm('Lưu thông tin nhà cung cấp thành công!');
        if (!option) {
            return;
        }
    }
