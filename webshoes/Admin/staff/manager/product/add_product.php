...
if (!empty($_POST['title'])) {
    ...
    if (isset($_POST['size']) && isset($_POST['quantity'])) {
        ...
        // Sử dụng ID sản phẩm đã tồn tại hoặc ID nhận từ POST
        $product_id = isset($id) ? $id : 0;

        // Chèn các size và quantity vào bảng product_size
        foreach ($sizes as $index => $size) {
            $quantity = $quantities[$index];

            // Chuyển đổi size và quantity về số nguyên
            $size = intval($size);
            $quantity = intval($quantity);

            // Chèn vào bảng product_size
            $sql_size = 'INSERT INTO product_size (product_id, size, quantity) 
    ...
    if ($id == '') {
        ...
        values ("' . $title . '","' . $price . '","' . $number . '","' . $thumbnail . '","' . $thumbnail_1 . '","' . $thumbnail_2 . '","' . $thumbnail_3 . '","' . $thumbnail_4 . '","' . $thumbnail_5 . '","' . $content . '","' . $id_category . '","' . $created_at . '","' . $updated_at . '")';
    } else {
        // Sửa danh mục
        $sql = 'update product set title="' . $title . '",price="' . $price . '",number="' . $number . '",thumbnail="' . $thumbnail . '",thumbnail_1="' . $thumbnail_1 . '",thumbnail_2="' . $thumbnail_2 . '",thumbnail_3="' . $thumbnail_3 . '",thumbnail_4="' . $thumbnail_4 . '",thumbnail_5="' . $thumbnail_5 . '",content="' . $content . '",id_category="' . $id_category .  '", updated_at="' . $updated_at . '" where id=' . $id;
    }
    execute($sql);
    ...
    <nav class="navbar show navbar-vertical h-lg-screen navbar-expand-lg px-0 py-3 navbar-light bg-white border-bottom border-bottom-lg-0 border-end-lg" id="navbarVertical">
        <div class="container-fluid">
            ...
            <div class="collapse navbar-collapse" id="sidebarCollapse">
                ...
                <ul class="navbar-nav">
                    <li class="nav-item">
                        <a class="nav-link" href="../../manager_product.php">
                    ...
                    <div class="table-responsive">
                        ...
                        <div class="panel-body">
                            <form method="POST" enctype="multipart/form-data">
                                ...
                                <div class="form-group">
                                    <label for="product_sizes">Size và Số Lượng:</label>
                                    <div id="size-quantity-container">
                                        <!-- Dòng nhập size và số lượng ban đầu -->
                                        <div class="size-quantity-row">
                                            <input required type="number" class="form-control" name="size[]" placeholder="Size (e.g., 38)" min="1">
